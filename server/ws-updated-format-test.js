// Updated format WebSocket test script 
...
const logFile = fs.createWriteStream('ws-updated-format-test.log');
...
log('Starting WebSocket connection test with UPDATED message format...');
...
ws.on('open', () => {
  ...
  // Wait for the session.created event before sending message
  setTimeout(() => {
    // Send a conversation item create message (correct format)
    ...
    log(`Sending conversation item: ${JSON.stringify(initialMessage)}`);
